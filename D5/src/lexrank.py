...
import traceback
import numpy as np
from sentence_compression import compress_sentence
import nltk
...
def clean(file_path, add_sentence_marker=False):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            elif stripped_line.startswith('DATELINE:') or stripped_line.startswith('DATE_TIME:') or stripped_line.startswith('DATETIME:') or stripped_line.startswith('<DATETIME>'):
                is_headline = False
                is_dateline = True
...
class LexRank:
    ...
    def get_summary(
        ...
        lex_scores, similarity_matrix = self.rank_sentences(
            sentences,
            threshold=threshold,
        ...
        summary_idx = sorted_ix[:summary_size]
        
        summary = [(i, sentences[i]) for i in summary_idx]
...
if __name__ == "__main__":
    ...
    for subdir in os.listdir(input_directory):
        if subdir > 'D1144':
            ...
            if len(cleaned_text_dict['HEADLINE'].split()) > 2:
                print("headline", cleaned_text_dict['HEADLINE'])
                sentences = [Sentence(text=cleaned_text_dict['HEADLINE'].lower(), properties=dict())] + sentences
            try:
                lxr = LexRank(sentences, stopwords=en_stop_words)
                summary, similarity = lxr.get_summary(sentences, summary_size=summary_length, threshold=similarity_threshold)
                print([(x[0], x[1].get_text()) for x in summary])
                if len(cleaned_text_dict['HEADLINE'].split()) > 2:
                    print(similarity[0])
                ...
                with open(os.path.join(output_directory, filename), "w") as output:
                    if len(cleaned_text_dict['HEADLINE'].split()) > 2:
                        output.write(cleaned_text_dict['HEADLINE'].lower().capitalize() + '\n')
                    sentences = [sentence.get_text() for idx, sentence in ordered_summary]
                    compressed_sentences = compress_sentence(sentences)
                    for compressed_sentence in compressed_sentences:
                        compressed_sentence = compressed_sentence.replace(" ##", "")
                        output.write(compressed_sentence + '\n')
